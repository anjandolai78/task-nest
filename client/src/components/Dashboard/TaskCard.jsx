...
const TaskCard = ({data}) => {
  ...
  return (
    <button
      className="bg-white rounded-xl p-4 w-full shadow-sm hover:shadow-md border border-transparent hover:border-gray-200 transition-all duration-300 group text-start"
      onClick={(event)=>showEditDiv(event,data._id)}
    >
      <div className="flex items-center justify-between mb-3">
        <h1 className="font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{data.title}</h1>
        <span className={`text-[10px] uppercase tracking-wider font-bold ${priorityClass} px-2 py-1 rounded-md`}>
          {displayPriority}
        </span>
      </div>
      ...
      {/* Date Section - Matching your 2nd Image */}
      <div className="flex items-center gap-6 border-t border-gray-50 pt-3">
        {formattedDate && (
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-medium text-gray-400 uppercase tracking-tight">Due</span>
        ...
        {formattedCreatedAt && (
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-medium text-gray-400 uppercase tracking-tight">Created</span>
            <div className="flex items-center gap-1.5 text-slate-600">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="text-xs font-semibold">{formattedCreatedAt}</span>
            </div>
          </div>
        )}
      </div>
